...
.PHONY: api
api: hypershift-api cluster-api cluster-api-provider-aws cluster-api-provider-gcp cluster-api-provider-ibmcloud cluster-api-provider-kubevirt cluster-api-provider-agent cluster-api-provider-azure cluster-api-provider-openstack karpenter-api externalplatform-example-api api-docs
...
# The AWS reference integration's own API. It is what an integrator copies, so it is
# generated from Go types the same way a real integration's would be, rather than kept as a
# hand-written pair of custom resource definitions that can drift apart.
#
# Its own module, so none of the invocations above reach it, and GOWORK=off because it
# resolves against its own pinned dependency set the way an integrator's build does.
EXTERNALPLATFORM_EXAMPLE_CRDS := $(abspath $(BIN_DIR))/externalplatform-example-crds
.PHONY: externalplatform-example-api
externalplatform-example-api: $(CONTROLLER_GEN)
	cd externalplatform && GOWORK=off $(CONTROLLER_GEN) object:headerFile="$(abspath hack/boilerplate.go.txt)" paths="./examples/aws/api/..."
	rm -rf $(EXTERNALPLATFORM_EXAMPLE_CRDS)
	cd externalplatform && GOWORK=off $(CONTROLLER_GEN) $(CRD_OPTIONS) paths="./examples/aws/api/..." output:crd:artifacts:config=$(EXTERNALPLATFORM_EXAMPLE_CRDS)
	@# Concatenated into the one file the README walks and a reader applies in a single
	@# kubectl apply, template first because that is the order the two types are met in.
	{ \
	  echo "# The integrator's own API, generated from externalplatform/examples/aws/api/v1alpha1."; \
	  echo "# Run 'make externalplatform-example-api' after editing those Go types. Do not edit this file."; \
	  echo "#"; \
	  echo "# Two types, and the relationship between their names is the contract: HyperShift takes the"; \
	  echo "# template resource a HostedCluster names, strips the \"templates\" suffix to get the instance"; \
	  echo "# resource, and instantiates the instance in the control plane namespace. Rename either one"; \
	  echo "# and HyperShift will not find the other."; \
	  echo "#"; \
	  echo "# Neither type is a Cluster API type. The Cluster API infrastructure object for this"; \
	  echo "# integration is Cluster API Provider AWS's own AWSCluster, which the provider creates."; \
	  cat $(EXTERNALPLATFORM_EXAMPLE_CRDS)/aws.example.hypershift.openshift.io_awshostedclustertemplates.yaml; \
	  cat $(EXTERNALPLATFORM_EXAMPLE_CRDS)/aws.example.hypershift.openshift.io_awshostedclusters.yaml; \
	} > externalplatform/examples/aws/manifests/01-crds.yaml
	rm -rf $(EXTERNALPLATFORM_EXAMPLE_CRDS)
